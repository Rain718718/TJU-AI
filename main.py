...
# 设置背景
img_folder = "images"
bg_img = pygame.image.load(os.path.join(img_folder, "image.png"))
...
x_min, y_min, x_max, y_max = 0, 0, 0, 0  # 搜索范围
movements = []  # 记录移动步骤
game_over = True
running = True
start = 1
...
# 绘制网格线
def print_gridlines(surf):
    screen.blit(background, back_rect)

    # 画棋盘的网格线
    rect_lines = [
        ((SPACE, SPACE), (SPACE, HEIGHT - SPACE)),
    ...
    for i in range(17):
        pygame.draw.line(
            surf, BLACK, (SPACE * (2 + i), SPACE), (SPACE * (2 + i), HEIGHT - SPACE)
        )
        pygame.draw.line(
            surf, BLACK, (SPACE, SPACE * (2 + i)), (HEIGHT - SPACE, SPACE * (2 + i))
        )
    # 画棋盘上的黑色标记
    circle_center = [
        (SPACE * 4, SPACE * 4),
...
# alpha-beta剪枝搜索
def ai_go():
    global x_min, x_max, y_min, y_max, color_flag, matrix
    time_start = time.time()
    evaluate_matrix = [
        [0 for i in range(SIZE + 2)] for j in range(SIZE + 2)
    ]  # 结点估值矩阵
    if step != 0:
        if step == 1:
            # 第一步下的位置
            if matrix[(SIZE + 1) // 2][(SIZE + 1) // 2] == 0:
                ...
                if (
                    matrix[(SIZE + 1) // 2][(SIZE + 1) // 2] != 0
                    and matrix[(SIZE + 1) // 2 + 1][(SIZE + 1) // 2 + 1] == 0
                ):
                    rx, ry = (SIZE + 1) // 2 + 1, (SIZE + 1) // 2 + 1
        else:
            min_tx1, min_ty1, max_tx1, max_ty1 = range_legal(x_min, y_min, x_max, y_max)
            evaluate_matrix = [
                [0 for i in range(SIZE + 2)] for j in range(SIZE + 2)
            ]  # 第一层的估值矩阵
            Max = -100000
            ...
            for i in range(min_tx1, max_tx1 + 1):
                for j in range(min_ty1, max_ty1 + 1):
                    ...
                    if matrix[i][j] == 0:
                        ...
                        min_tx2, min_ty2, max_tx2, max_ty2 = range_legal(
                            min_tx1, min_ty1, max_tx1, max_ty1
                        )
                        ...
                        for ii in range(min_tx2, max_tx2 + 1):
                            for jj in range(min_ty2, max_ty2 + 1):
                                ...
                                if matrix[ii][jj] == 0:
                                    ...
                                    [list_h, list_v, list_s, list_b] = get_list(
                                        ii, jj, -color_flag
                                    )
...
# 开始界面显示
def init_UI(surf):
    global win_flag, movements, step, matrix, x_min, y_min, x_max, y_max, game_over, start
    if start == 1:
        if win_flag != 0:
            root = tkinter.Tk()
            root.withdraw()
